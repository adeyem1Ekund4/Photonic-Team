...
from utils.green_corner_detection import detect_green_corners, order_points, CornerTracker, detect_corners_in_mask, OPTIMAL_GREEN_PARAMS  # Add OPTIMAL_GREEN_PARAMS here
...
def main():
    ...
    # Variables for adaptive frame processing
    frame_skip_threshold = 0.05  # Skip if processing takes >50ms
    last_processing_time = 0.01  # Initial value
    ...
    while running:
        try:
            ...
            # Adaptive processing based on previous frame time
            if last_processing_time > frame_skip_threshold and config_manager.get_detection_config().get("enable_frame_skip", False):
                # Process at lower resolution when under high load
                processing_scale = 0.5  # Use half resolution when system is struggling
                
                # Check for quit key even when in reduced processing mode
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):
                    running = False
                    break
            else:
                # Use normal processing scale from config
                processing_scale = config_manager.get_detection_config().get("processing_scale", 0.75)
            ...
            # Create a separate copy for processing with optimized scale
            ...
            if debug_mode:
                ...
                param_text = [
                    ...
                    f"Processing time: {last_processing_time*1000:.1f}ms"  # Show processing time
                ]
            ...
            # Update the processing time for next iteration
            last_processing_time = frame_time
